...
namespace previously exposed so downstream callers can depend exclusively on
``viterbo.modern``.  The implementations mirror the legacy semantics but are
...
def symplectic_product(
    ...
    *,
    ...
    if first.shape[0] != second.shape[0]:
        msg = "Vectors must share the same dimension."
        raise ValueError(msg)
